...
Talvez adicionar os termos de interação entre explicativas sejam uma solução para o problema. Isso irá ocorrer se a relação entre as explicativas não for linear.

Teste para verificação da existência de multicolinearidade:
...
Comparando curvas Normal e Gamma para log da Face:

```{r message=FALSE, warning=FALSE}
denscomp(list(fitdist(log(dados$Face), "lnorm"), fitdist(log(dados$Face), "gamma")), legendtext=c("Normal ajustada", "Gamma ajustada"), fitcol = c("blue", "orangered"), fitlty= c(3, 1), demp=T, main="Histograma do log de 'Face' e densidades ajustadas", xlegend=7, ylegend=0.2)
```
